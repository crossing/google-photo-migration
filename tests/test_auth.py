...
def test_get_credentials_existing_token(tmp_path):
    token_path = tmp_path / "token.json"
    client_secrets = tmp_path / "client_secrets.json"
    client_secrets.write_text("{}")
    
    # Create a mock token file
    mock_creds_json = '{"token": "fake-token", "refresh_token": "fake-refresh", "token_uri": "https://oauth2.googleapis.com/token", "client_id": "fake-id", "client_secret": "fake-secret", "scopes": ["https://www.googleapis.com/auth/drive.readonly"]}'
    token_path.write_text(mock_creds_json)
    ...
    with patch('src.auth.google_auth.Credentials') as mock_creds_class:
        ...
        mock_creds_class.from_authorized_user_file.assert_called_once_with(str(token_path), DRIVE_SCOPES)
...
def test_get_credentials_new_flow(tmp_path):
    ...
    with patch('src.auth.google_auth.InstalledAppFlow') as mock_flow_class, \
        ...
        assert creds == mock_creds
        mock_flow_class.from_client_secrets_file.assert_called_once_with(str(client_secrets), PHOTOS_SCOPES)
